...
module.exports = function(options) {
  return async function(ctx, next) {
    ...
    if(ctx.headers['x-forwarded-proto'] == 'https'){
      logoutPath = 'https://' + ctx.headers['host'] + options.paths.logout + '?service=' + encodeURIComponent(ctx.headers.referer || ctx.origin);
    }else{
      logoutPath = options.path + options.paths.logout + '?service=' + encodeURIComponent(ctx.headers.referer || ctx.origin);
    }
    // Send the user to the official campus-wide logout URL
    return ctx.response.redirect(logoutPath);
  }
}